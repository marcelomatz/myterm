...
$DefaultPfx  = Join-Path $WinDir 'myterm-dev.pfx'
$DefaultPass = 'myterm-dev-2026'

if ($PfxPath -eq '') { $PfxPath = $DefaultPfx }
if ($PfxPass -eq '') { $PfxPass = $DefaultPass }
if ($env:MYTERM_CERT_PATH) { $PfxPath = $env:MYTERM_CERT_PATH }
if ($env:MYTERM_CERT_PASS) { $PfxPass = $env:MYTERM_CERT_PASS }
...
if (-not (Test-Path $PfxPath)) {
    ...
    $cert = New-SelfSignedCertificate `
        -Subject           'CN=MyTerm Dev, O=Marcelo Matzembacher, C=BR' `
...
if ($signtool -and (Test-Path $exe)) {
    WriteStep 'Assinando myterm.exe'
...
# ─── 8. Limpar cache de icones do Windows ────────────────────────────────────
