<script lang="ts">
  ...
  import {
    COLOR_PRESETS, DEFAULT_SETTINGS,
  ...
  function buildThemeVars(presetId: string): Record<string, string> {
    ...
    return {
      ...
      '--tset-cyan':      bCy,
      '--tset-accent':    grn,
      '--tset-dim':       colorWithAlpha(fg, 0.20),
  ...
  function resetToDefaults() {
    ...
    saveSettings(draft);
    applySettingsToAll(getLeaves(), draft);
    panelFont = draft.fontFamily;
    themeVars = buildThemeVars(draft.colorPresetId);
    onRebuild();
  }

  // ── mock preview HTML ────────────────────────────────────────────────────
  function s(cls: string, txt: string) { return `<span class="${cls}">${txt}</span>`; }
  function L(...parts: string[]) { return `<div class="pv-line">${parts.join('')}</div>`; }
  function prompt(path: string) {
    return s('pv-user','user') + s('pv-at','@') + s('pv-host','myterm') + s('pv-sep',':') +
...
<div class="tset-wrap" style="{styleStr};font-family:{panelFont}">
    ...
    <div class="tset-panel">
      ...
      <div class="tset-section">
        ...
        <div class="tset-row">
          ...
          <span class="tset-val">
            <select class="tset-select" value={draft.fontFamily}
              onchange={(e) => onFontFamily((e.target as HTMLSelectElement).value)}>
              {#each FONT_OPTIONS as opt}
                <option value={opt.value} selected={draft.fontFamily === opt.value}>{opt.label}</option>
              {/each}
            </select>
          </span>
        </div>
        <div class="tset-row">
          ...
          <span class="tset-val">
            <span class="tset-range-wrap">
              <input type="range" class="tset-range" min="8" max="28" step="1"
                value={draft.fontSize}
                oninput={(e) => onFontSize(Number((e.target as HTMLInputElement).value))} />
        ...
        <div class="tset-row">
          ...
          <span class="tset-val">
            <span class="tset-range-wrap">
              <input type="range" class="tset-range" min="1.0" max="2.0" step="0.05"
                value={draft.lineHeight}
                oninput={(e) => onLineHeight(Number((e.target as HTMLInputElement).value))} />
        ...
        <div class="tset-row">
          ...
          <span class="tset-sep"> │ </span>
          <span class="tset-val">
            <select class="tset-select" value={draft.cursorStyle}
              onchange={(e) => onCursorStyle((e.target as HTMLSelectElement).value)}>
              {#each ['block','bar','underline'] as style}
                <option value={style} selected={draft.cursorStyle === style}>{style}</option>
              {/each}
            </select>
          </span>
        </div>
        <div class="tset-row">
          ...
          <span class="tset-sep"> │ </span>
          <span class="tset-val">
            <button class="tset-toggle{draft.cursorBlink ? ' on' : ''}"
      ...
      <div class="tset-section">
        ...
        <div class="tset-row">
          ...
          <span class="tset-sep"> │ </span>
          <span class="tset-val">
            <select class="tset-select" value={draft.colorPresetId}
              onchange={(e) => onColorPreset((e.target as HTMLSelectElement).value)}>
              {#each COLOR_PRESETS as p}
                <option value={p.id} selected={draft.colorPresetId === p.id}>{p.name}</option>
              {/each}
            </select>
          </span>
        </div>
      ...
      <div class="tset-section">
        ...
        {#if shells.length > 0}
          <div class="tset-row">
            ...
            <span class="tset-sep"> │ </span>
            <span class="tset-val">
              <select class="tset-select" value={draft.defaultShell}
                onchange={(e) => onDefaultShell((e.target as HTMLSelectElement).value)}>
                <option value="">（auto）</option>
                {#each shells as sh}
                  <option value={sh} selected={draft.defaultShell === sh}>{sh}</option>
                {/each}
              </select>
            </span>
          </div>
        {:else}
          <div class="tset-note">  # shell detection unavailable — using auto</div>
        {/if}
      </div>
      ...
      <div class="tset-section">
        ...
        <div class="tset-row">
          ...
          <span class="tset-val">
            <span class="tset-range-wrap">
              <input type="range" class="tset-range" min="500" max="10000" step="100"
                value={draft.scrollback}
                oninput={(e) => onScrollback(Number((e.target as HTMLInputElement).value))} />
        ...
        <div class="tset-row">
          ...
          <span class="tset-sep"> │ </span>
          <span class="tset-val">
            <button class="tset-toggle{draft.copyOnSelect ? ' on' : ''}"
      ...
      <div class="tset-footer">
        <button class="tset-reset-btn" onclick={resetToDefaults}>[ reset to defaults ]</button>
      </div>
    </div>
  </div>
  ...
  <div class="tset-preview-pane">
    <div class="tset-preview-hdr">── LIVE PREVIEW ──────────────────────────────────────────</div>
    <div class="tset-preview-term"
